class Play::Pandemic < Play::Base
  ...
  def player_draw(player)
    ...
    player.update_column(:action_phase, "infect")
    ...
    if epidemic?
      ...
      if !session.completed?
        ...
        new_city.deal
        new_city.update(session_deck: decks.find_by(key: "infection-#{epidemic_count - 1}"))
      end
    end
  ...
  def player_infect(player)
    ...
    if one_quiet_night?
      session.update_column(:special_game_phase, nil)
    else
  ...
  def player_submit_trade(player, params)
    ...
    if card.present? && receiving_player.present?
      card.update(player: receiving_player)
      check_hand_limit(receiving_player)
    end
    player.update_column(:action_phase, player.action_phase_revert)
  end
  ...
  def player_start_turn(player)
    player.update_column(:action_phase, "draw")
  end
  ...
  def card_played(session_card)
    ...
    when "resilient pop", "one quiet night", "forecast"
      session.update_column(:special_game_phase, card.name)
    end
    if forecast?
      forecast_deck = decks.create(key: "forecast")
      cards = deck_cards("infection").first(6)
      cards.each { |card| card.update(session_deck: forecast_deck) }
    end
  ...
  def card_dealt(session_card)
    if session_card.card.key == "infection"
      session_card.update(session_deck: decks.find_by(key: "infection-#{epidemic_count}"))
      if forecast?
        session.update_column(:special_game_phase_timer, session.special_game_phase_timer + 1)
  ...
  def check_hand_limit(player)
    if player.action_phase == "discard"
      player.update_column(:action_phase, player.action_phase_revert) if player.cards.active.count <= 7
  ...
  def infect_next(count = 1)
    count.times do
      ...
      infecting_city.deal
      infecting_city.update(session_deck: decks.find_by(key: "infection-#{epidemic_count}"))
    end
  end
  ...
  def reset_forecast
    if forecast_deck.empty?
      ...
      forecast_deck.each { |card| card.update(session_deck: fake_forecast_placement) }
    end
    session.update(special_game_phase: nil, special_game_phase_timer: 0)
    session.advance_turn
  end
  ...
  def create_infection_decks
    ...
    infection.cards.shuffle.first(9).each do |card|
      card.update(session_deck: infection_discard)
      card.deal
    end
  end
