class SessionCard < ApplicationRecord
  ...
  def deal(player = nil)
    SessionCard.transaction do
      ...
      session.engine.card_dealt(self)
    end
  end
  ...
  def play(affected_player = nil)
    SessionCard.transaction do
      ...
      update_column(:played_at, Time.zone.now)
      SessionFrame::Create.(session,
        action: "card_played",
      ...
      session.engine.card_played(self)
    end
  end
  ...
  def discard(player = nil)
    SessionCard.transaction do
      ...
      session.engine.card_discarded(self)
    end
  end
  ...
  def valid_action
    ...
    session.engine.card_valid_action(self)
  end

  def playable?
    active? && session.engine.card_playable?(self)
  end

  def playable_out_of_turn?
    session.engine.card_playable_out_of_turn?(self)
  end

  def discardable?
    !discarded? && session.engine.card_discardable?(self)
  end

  def tradeable?
    session.engine.card_tradeable?(self)
  end
end
